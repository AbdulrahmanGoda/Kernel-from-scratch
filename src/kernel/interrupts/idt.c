...
void initIdt(){
    ...
    memorySet(&idt_entries, 0, sizeof(struct idt_entry_struct)* 256);

    /*
    Hey there!! this is GODA, I want to talk about "Programmable Interrupt Controller" or PIC.
    There are two PIC chips responsible for interrupts programming (master and slave). 
    one is going to set at addresses:
    0x20 for commands and 0x21 for data
    the second is going to set at addresses:
    0xA0 for commands and 0xA1 for data
    These must be initialized in order to use interrupts.
    */
    //command initialization
    outPortB(0x20, 0x11);
    outPortB(0xA0, 0x11);
    //offset to data registers
    outPortB(0x21, 0x20);
    outPortB(0xA1, 0x28);
    //cascaded mode
    outPortB(0x21,0x04);
    ...
    0x08 = 0000 1000 is the segment selector for the code segment.
    0x8B = 1000 1110 Present bit and DPL.
    */
    //Exceptions and NMI
...
void setIdtGate(uint8_t num, uint32_t base, uint16_t sel, uint8_t flags){
    ...
    idt_entries[num].flags = flags | 0x60; //0110 0000
}
...
void isr_handler(struct InterruptRegisters* regs){
    if (regs->int_no < 32){
        ...
        terminal_writestring("Exception! System is Halted\n");
        for (;;);
    }
}
